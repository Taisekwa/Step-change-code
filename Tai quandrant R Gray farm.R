...
## Read data from file
data <- read.csv("C:/Users/chikazhet/Gray.csv")
...
data2 <- data %>% filter(data$Season== "2021-22")%>%
...
ggplot(data2, aes(N_surplus,Operating_profit_ha, label=Farm)) +
 ...
 col="blue") + geom_vline(xintercept=median(data$N_surplus), 
  ...
  annotate("text", x = 250, y = 1250, label = "D", col = "blue", size = 10) +
   ...
   geom_text(aes(label=ifelse(Farm=="Gray",as.character(Farm),'')),hjust=0,vjust=0,fontface="bold") +
...
tai <- data2 %>% filter(data2$district== "Hauraki") %>%
  dplyr::mutate(Profitability = 
                  dplyr::case_when(
                    Operating_profit_ha > median(Operating_profit_ha) ~ "Above median",
                    Operating_profit_ha <= median(Operating_profit_ha) ~ "Below median"))
...
data3 <- data %>% filter(data$Season== "2022-23")%>%
  dplyr::mutate(Profitability = 
                  dplyr::case_when(
                    Operating_profit_ha > median(Operating_profit_ha) ~ "Above median",
                    Operating_profit_ha <= median(Operating_profit_ha) ~ "Below median"))
...
ggplot(data3, aes(methane,Operating_profit_ha,label=Farm)) +
  ...
  annotate("text", x = 5.5, y = 7500, label = "A", col = "blue", size = 10) +
  
  annotate("text", x = 15, y = 7500, label = "B", col = "blue", size = 10)+
  
  annotate("text", x = 5.5, y = 1250, label = "C", col = "blue", size = 10) +
...
# methane emission intensity quandrant graph with AAA, farm 1 and 2 positions 2022/23 Waikato.

ggplot(data3, aes(intensity,Operating_profit_ha,label=Farm)) +
  
  geom_point(size=1.5, col="red") +
  
  geom_hline(yintercept=median(data3$Operating_profit_ha), col="blue") + 
  
  geom_vline(xintercept=median(data3$intensity), col="blue")+
  ...
  xlab("Methane emission intensity (kg CO2 equiv./kgMS/yr)") + 
  
  ylab("Dairy operating profit ($/ha)") + 
  
  labs(title="Operating profit vs methane emissions intensity 2022-23 season") + 
